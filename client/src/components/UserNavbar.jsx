...
export default function UserNavbar() {
  ...
  const Logout = async (e) => {
    try {
      const response = await axiosInstance.get(baseURL + "user/logout")
  ...
  return (
    ...
    <nav dir='ltr' className="navbar navbar-expand-lg" style={{ boxShadow: "1px 1px 5px 1px #aaa", background: '#05a' }}>
      <div className="container">
        ...
        <div className="collapse navbar-collapse" id="navbarNav">
          <ul dir='rtl' className="navbar-nav ml-auto">
            {user ? (
              <>
                <button onClick={Logout} className="btn btn-sm btn-danger ml-3">
                  <FontAwesomeIcon icon={faRightToBracket} />
                </button>
                <CustomLink className="nav-link" to="/user/profile">{user && user.user ? user.user.name : ''}</CustomLink>
              </>
            ) : (
              <CustomLink className="nav-link" to="/user/login">ورود</CustomLink>
            )}
