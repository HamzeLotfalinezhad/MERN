...
export default function EditFailure(props) {
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      if (response && response.status === 204) navigate("/user/profile")
    } catch (error) {
      setLoading(false)
    } finally {
      setLoading(false)
  ...
  const confirmFcn = async () => {
    const response = await axiosInstance.delete('/record/delete', { data: { _id: record._id } });
  ...
  return (
    <>
      <ConfirmModal
        ...
        heading='حذف رکورد'
        body='پس از حذف رکورد امکان بازیابی وجود ندارد!'
      ...
      <div className="row" style={{ padding: '0px 5px 15px 5px' }}>
        ...
        <div className="col-md-6 col-sm-12 mx-auto mt-5">
          ...
          <hr style={{ marginTop: '5px' }}></hr>
          <ButtonSubmit onClick={submitForm} loading={loading} className="btn-sm btn-warning" style={{ width: '100%' }}>ویرایش رکورد</ButtonSubmit>
